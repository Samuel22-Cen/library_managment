...
class Book {
    ...
    public function delete() {
        try {
            ...
            // 1. 🗑️ FIRST DELETE RELATED BORROW RECORDS
            $delete_borrow_query = "DELETE FROM borrow_records WHERE book_id = ?";
            $borrow_stmt = $this->conn->prepare($delete_borrow_query);
            $borrow_stmt->bindParam(1, $this->id);
            $borrow_stmt->execute();
            
            // 2. 🗑️ THEN DELETE RELATED RESERVATIONS  
            $delete_reservations_query = "DELETE FROM book_reservations WHERE book_id = ?";
            $reservation_stmt = $this->conn->prepare($delete_reservations_query);
            $reservation_stmt->bindParam(1, $this->id);
            $reservation_stmt->execute();
            
            // 3. 🗑️ FINALLY DELETE THE BOOK ITSELF
